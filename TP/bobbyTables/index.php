...
$existing_characters = ["wizard", "mage", "oc"];
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (in_array($raw_name, $existing_users)){
    echo "Ce nom est pris. <br>".$validation_error;
  } else {
    $name = $raw_name;
  }
  ...
  if (in_array($raw_character, $existing_characters)){
    ...
    echo "Vous devez choisir un magicien, un mage ou un orc. <br>".$validation_error; 
  }
  ...
  if (filter_var($raw_birth_year, FILTER_VALIDATE_INT, $options)) {
    $birth_year = $raw_birth_year;
         ...
         echo "Cela ne peut pas être votre année de naissance. <br>".$validation_error;
    }
